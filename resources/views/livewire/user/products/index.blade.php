<div class="mt-5">
    <div class="row justify-content-between align-items-center">
        <div class="col-md-6 col-lg-8 d-flex justify-content-between align-items-center">
            <h3>Products</h3>
            <a href="#" wire:click.prevent="$emitTo('user.products.edit','load')" class="btn btn-primary text-white"
        ...
        <table class="table table-bordered m-0">
            ...
            <tbody>
                @foreach ($products as $k => $product)
                    <tr wire:key="{{ $k }}">
                        <td>{{ $loop->index + 1 }}</td>
                        <td><a href="{{ route('user.product.detail', $product) }}"
                                class=""><div class="d-flex flex-column justify-content-center w-100">
                                    <img src="{{ asset('storage/' . $product->image) }}" width="200px" class=" mb-2 rounded" alt="">
                                    {{ $product->name }}
                                </div>
                        ...
                        <td>
                            ...
                            <a href="#"
                                wire:click.prevent="$emitTo('user.products.edit','load',{{ $product }})"
                                class="btn btn-default text-success"><i class="fa fa-pen"></i> Edit</a>
                            <a href="#"
                                wire:click.prevent="$emitTo('user.products.delete','load',{{ $product }})"
